...
// Static hints for known category keywords — partial, case-insensitive match
const CAT_COLOR_HINTS = [
  { keywords: ["dining", "food", "eat", "cafe", "restaurant", "swiggy", "zomato", "tiffin", "blinkit", "grocer"], color: "#C96442" },
  { keywords: ["invest", "mutual", "fund", "sip", "zerodha", "groww", "stock", "equity"], color: "#2D6A4F" },
  { keywords: ["commute", "transport", "cab", "metro", "ola", "uber", "rapido", "travel"], color: "#B5860D" },
  { keywords: ["subscription", "streaming", "netflix", "spotify", "youtube", "prime", "saas", "google one"], color: "#7B5EA7" },
  { keywords: ["util", "electric", "water", "bescom", "housing", "rent", "maintenance", "bbmp", "property"], color: "#3B82C4" },
  { keywords: ["shop", "amazon", "lifestyle", "fashion", "flipkart", "myntra"], color: "#D4696B" },
  { keywords: ["insur", "lic", "health", "protect", "premium"], color: "#5B7FA6" },
  { keywords: ["family", "transfer", "remit", "phonepe", "upi", "mom", "dad", "parent"], color: "#8A6D3B" },
  { keywords: ["income", "salary", "payout", "credit"], color: "#2D6A4F" },
  { keywords: ["medical", "health", "pharma", "doctor", "hospital"], color: "#E07B54" },
  { keywords: ["fuel", "petrol", "diesel", "vehicle"], color: "#C4973A" },
  { keywords: ["education", "course", "school", "college", "udemy"], color: "#4A7FA5" },
];

// Deterministic fallback palette — assign by index so each unknown category gets a unique color
const FALLBACK_PALETTE = [
  "#C96442", "#2D6A4F", "#B5860D", "#7B5EA7", "#3B82C4",
  "#D4696B", "#5B7FA6", "#8A6D3B", "#E07B54", "#4A7FA5",
  "#C4973A", "#6B8E6B",
];
...
function getCatColor(categoryName) {
  if (!categoryName) return FALLBACK_PALETTE[0];
  if (_colorCache[categoryName]) return _colorCache[categoryName];

  const lower = categoryName.toLowerCase();

  // Try keyword hints first
  for (const { keywords, color } of CAT_COLOR_HINTS) {
  ...
  // Assign next fallback color deterministically
  const color = FALLBACK_PALETTE[_fallbackIdx % FALLBACK_PALETTE.length];
  _fallbackIdx++;
  _colorCache[categoryName] = color;
  return color;
}
...
function Dashboard({ data, onReset }) {
  ...
  const analysis = data || (() => {
    ...
    return {
      ...
      period: "October 2026",
      idle_cash: {
        monthly_burn: totalBurn, total_income: totalIncome, balance, investable_surplus: surplus,
          ...
          <ResponsiveContainer width="100%" height={210}>
            <PieChart>
              <Pie data={pieData} cx="50%" cy="50%"
                ...
                {pieData.map((c, i) => (
                  <Cell key={i} fill={getCatColor(c.name)} />
                ))}
              </Pie>
          ...
          <div className="cat-list">
            {categories.map((c, i) => (
              <div className="cat-row" key={i}>
                <div className="cat-dot" style={{ background: getCatColor(c.name) }} />
                <span className="cat-name">{c.name}</span>
                <div className="cat-bar-wrap">
                  <div className="cat-bar" style={{ width: `${(c.total / maxCat) * 100}%`, background: getCatColor(c.name) }} />
                </div>
                ...
                <tr key={i}>
                  ...
                  <td>
                    <span className="cat-pill" style={{
                      background: getCatColor(t.category) + "22",
  ...
  const handleAnalyze = useCallback(async (file) => {
    ...
    try {
      ...
      const [response] = await Promise.all([
        fetch(`https://finnius-ai-xs9g.onrender.com/api/analyze`, {
          method: "POST",
          body: formData,
        }),
